...
It only requires a conversion back to JSON and output to HDFS instead of file. Note we also added a label _"json:"_ at this step of the stream. We'll use that for our next stream. +
From the XD shell window, type:
 +
[source,bash]
----
xd:>stream create transformToHDFS --definition "http --port=9020 | splitter --expression=#jsonPath(payload,'$.Values') | filter --expression=#jsonPath(payload,'$.Y').equals(0) | json: object-to-json | hdfs --directory=/xd --fileName=output" --deploy
...
xd:>stream create gemfireTap --definition "tap:stream:transformToHDFS.json > gemfire-json-server --useLocator=true --host=localhost --port=10334 --regionName=ValuesFromXD --keyExpression=payload.getField('key')" --deploy
...
or you can also see the values using the Rest API URL [http://localhost:8888/gemfire-api/v1/ValuesFromXD] 


Convert to CSV for Hawq to read:
groovy
csv = payload.get('X')+','+payload.get('Y')+','+payload.get('Z')+","+payload.get
('key')
return csv
