...
== Prerequisites
If using AWS, ensure the lab instructions are open on the AWS instance: +
...
If the work space has already been cloned from Git hub, skip the following step.
To get this project run the following command in the terminal: +
[source]
----
...
----
Once the project is cloned, ensure that the terminal session is running in the labs directory.
[source]
----
...
----
Congratulations, you now have the beginning of a Gemfire cluster. Wait for the previous command to finish before preceding to the next step. +
...
locatorA | 192.168.75.9(locatorA:72292:locator)<v0>:58229
----
image::/labs/01-gemfire/images/pulse-full-cluster-view.png[]
...
image::01-gemfire/images/pulse-data-browser.png[]
...
To get the number of records in the region, type and run this OQL query in gfsh or the data browser:
[source,sql]
----
