...
2. Partitioned - when a client sends data to a server and puts a key/value into this type of Region, a hashing policy is performed on the key and using this one server is selected to hold that key/value. In most cases redundant copies are made. +
