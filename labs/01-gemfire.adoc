...
2. Partitioned - when a client sends data into this type of Region, a hashing policy is performed on the key and using the result server is selected to hold that key/value. In most cases redundant copies are made. +
...
----
In this lab, to get a better feel for the process, Regions will be created on the fly using gfsh. The downside of this approach is the Regions are not persistant should a member need to be restarted. The xml configuration options is best for this. +
