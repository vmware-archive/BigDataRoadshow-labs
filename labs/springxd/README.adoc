...
[source,bash]
----
xd:>stream create transformToHDFS --definition "http --port=9020 | splitter --expression=#jsonPath(payload,'$.Values') | filter --expression=#jsonPath(payload,'$.Y').equals(0) | transform --script='file:/home/gpadmin/Desktop/BigDataRoadshow/labs/transform.groovy'| hdfs --directory=/xd --fileName=output " --deploy
...
link:http://localhost:8888/gemfire-api/v1/ValuesFromXD[http://localhost:8888/gemfire-api/v1/ValuesFromXD]
